...
(HGKM-DEF-MAC CHR2SYM (L)
   (LIST 'INTERN (LIST 'COERCE (LIST 'MAKE-ARRAY (LIST 'LENGTH L)
		       :INITIAL-CONTENTS
		       (LIST 'MAPCAR '(FUNCTION CODE-CHAR) L)) ''STRING)))
...
(HGKM-DEF-MAC CHR2STR (L)
   (LIST 'COERCE (LIST 'MAKE-ARRAY (LIST 'LENGTH L)
	 :INITIAL-CONTENTS (LIST 'MAPCAR '(FUNCTION CODE-CHAR) L)) ''STRING))
...
(HGKM-DEF-FUN EXPLODEN1 (X Y N LEN)
  (COND ((> N LEN) (REVERSE Y))
	(T (EXPLODEN1 X (CONS (CHAR-CODE (CHAR X N)) Y) (+ 1 N) LEN))))
