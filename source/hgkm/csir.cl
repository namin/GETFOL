...
(DEFMACRO SYM-MK (L)
 `(MULTIPLE-VALUE-BIND (SYM RES)
   (INTERN (COERCE (MAKE-ARRAY (LENGTH ,L)
            :INITIAL-CONTENTS (MAPCAR (FUNCTION CODE-CHAR) ,L)) 'STRING))
   SYM))


(DEFUN CSIR (STR CHR)
  (PROG (TOK END NEWEND CH)
    (SETQ TOK (LIST (CHAR-CODE CHR)))
    (SETQ END TOK)
  L (SETQ CH (READ-CHAR STR NIL 'EOF-FLAG))
    (COND ((EQ CH 'EOF-FLAG)
           (RETURN (SYM-MK TOK)))
          ((= (CHRTAB-GET (CHAR-CODE CH)) ID)
           (SETQ NEWEND (LIST (CHAR-CODE CH)))
           (RPLACD END NEWEND)
           (SETQ END NEWEND)
           (GO L))
          (T
